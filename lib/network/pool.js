...
function RenterPool(options, messaging) {
  ...
  this.manager = new storj.StorageManager(new MongoAdapter(options.storage));
 ...
 */
RenterPool.prototype.getStorageOffer = function(contract, bl, callback) {
  log.info('[renterpool] getting storage offer');
  this._send('getStorageOffer', [contract.toObject(), bl], callback);
};
 ...
 * Asks a minion for her connected contacts
 */
RenterPool.prototype.getMirrorNodes = function(sources, dests, callback) {
  log.info('[renterpool] getting connected contacts');
  this._send('getMirrorNodes', [sources, dests], callback);
};
