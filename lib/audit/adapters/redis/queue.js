...
AuditQueue.prototype.add = function(audits, callback) {
  var command = [this.rKeys.backlog];
...
AuditQueue.prototype.populateReadyQueue = function(start, stop, callback) {
  ...
  var command = [['ZREMRANGEBYSCORE', this.rKeys.backlog, start, stop]];

  this.client.watch(this.rKeys.backlog, function(watchErr, watchReply) {
    ...
    self._get(start, stop, function(err, resp) {
      if(resp.length === 0) {
        log.info('Audit:' + self._uuid
          ...
          + ': ' + self.rKeys.backlog + ' empty'
        );

        return callback(null, false);
      } else {
        var pushCommand = ['RPUSH', self.rKeys.ready];
...
AuditQueue.prototype.getReadyQueueLength = function(callback) {
  this.client.LLEN(this.rKeys.ready,
    function(err, result) {
      if(err) return callback(err);
...
AuditQueue.prototype.pushResultQueue = function(audit, hasPassed, callback) {
  ...
  var command = [
    ['LREM', self.rKeys.pending, 1, audit],
    ['SADD', self.rKeys[finalQueue], audit]
  ];

  this.client.watch(this.rKeys.pending, function(watchErr, watchReply) {
    console.log('pushRWatch')
    console.log(watchErr)
    console.log(watchReply)
    self.client.multi(command).exec(function(err, arrResp) {
      console.log('pushRMulti')
      console.log(err)
      console.log(arrResp)
      if(err) return callback(err);
      if(arrResp === null) {
...
AuditQueue.prototype._get = function(start, stop, callback) {
  let command = [this.rKeys.backlog, start, stop];
  console.log(command)
