...
Monitor.prototype._transferShard = function(shard, state, callback) {
  ...
  this.network.getRetrievalPointer(source, contract, (err, pointer) => {
    ...
    StorageEvent.save(monitorStorageEvent, (err) => {
      if (err) {
      ...
      this.network.ping(contact, (err) => {
        if (err) {
          ...
          if (contactData.timeoutRate >= timeoutRateThreshold) {
            ...
            //TODO: Pull constants from service-storage-models
            contact.recordPoint(POINTS.TRANSFER_FAILURE);
            //TODO: Find shard hash in the callback stack
            const StorageEvent = this.storage.models.StorageEvent;
            StorageEvent.update(
              {
                farmerId: contact._id, 
                shardHash: TODO
              },
              {
                $currentDate: {
                ...
                if (err) {
                  log.warn("Unable to update storage event with store end date: %s",
                    err.message);
                }
              });
          }
