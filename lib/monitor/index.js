...
Monitor.prototype._createStorageEvent = function(token, shardHash, source, destination) {
  ...
  const storageEvent = new StorageEvent({
    token: token,
    frame: null, // TODO
    user: source.nodeID, // farmer that already has the mirror
    farmer: distination.nodeID, // farmer storing the new mirror
    timestamp: Date.now(),
    downloadBandwidth: 0,
    storage: 0,
    farmerStorage: bytes,
...
Monitor.prototype._transferShard = function(shard, state, callback) {
  ...
  this.network.getRetrievalPointer(source, contract, (err, pointer) => {
    ...
    let shardHash = shard.hash;
    this._createStorageEvent(token, shardHash, source, destination);
...
Monitor.prototype._markStorageEventsEnded = function(contact) {
  ...
  StorageEvent.update({
    ...
    if (err) {
      log.error('Unable to update farmer %s storage events with storeEnd, ' +
      ...
      this.network.ping(contact, (err) => {
        if (err) {
          ...
          if (contactData.timeoutRate >= timeoutRateThreshold) {
            ...
            this._giveOfflinePenalty(contact);
            this._markStorageEventsEnded(contact);
          }
