...
Monitor.prototype.run = function() {
  ...
  const cursor = Contact.find({}).limit(limit).sort({lastSeen: -1});
  cursor.exec((err, contacts) => {
    ...
    async.eachLimit(contacts, pingConcurrency, (contact, next) => {

      this.network.ping(contact, (err) => {
        if (err) {
          fail += 1;
          log.error('Farmer %s failed ping, reason: %s',
                    contact.id, err.message);
        } else {
          success += 1;
        }
        // TODO replicate and replace associated contracts on
        // repeated failures
        next();
      });
