...
const {getPOWMiddleware, getChallenge, getTarget} = require('../middleware/pow');
...
function ContactsRouter(options) {
  ...
  if (options.pow && options.pow.retargetPeriod) {
    this.powOpts.retargetPeriod = options.pow.retargetPeriod;
  }
  if (options.pow && options.pow.retargetCount) {
    this.powOpts.retargetCount = options.pow.retargetCount;
  }
...
ContactsRouter.prototype.createChallenge = function(req, res, next) {
  getChallenge(this.redis, this.powOpts, function(err, data) {
    if (err) {
...
ContactsRouter.prototype.createContact = function(req, res, next) {
  ...
  Contact.record({
    ...
    // TODO check if created and use 200 and 201 status codes
    // TODO check that this is idempotent
    res.status(201).send({
...
ContactsRouter.prototype._definitions = function() {
  return [
    ['GET', '/contacts', this.getLimiter(limiter(1000)), this.getContactList],
    ['GET', '/contacts/:nodeID', this.getLimiter(limiter(1000)), this.getContactByNodeID],
    ['PATCH', '/contacts/:nodeID', this.getLimiter(limiter(100)), rawBody, authFarmer, this.patchContactByNodeID],
    ['POST', '/contacts', this.getLimiter(limiter(10)), this.checkPOW, rawBody, authFarmer, this.createContact],
    ['POST', '/contacts/challenges', this.getLimiter(limiter(100)), rawBody, authFarmer, this.createChallenge]
  ];
};
