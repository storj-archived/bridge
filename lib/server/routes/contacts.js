...
/**
 * Returns the farmer profile for the given nodeID
 * @param {http.IncomingMessage} req
 * @param {http.ServerResponse} res
 * @param {Function} next
 */
ContactsRouter.prototype.getProfileByNodeID = function(req, res, next) {
  const FarmerProfile = this.storage.models.FarmerProfile; 

  FarmerProfile.findOne({ farmerId: req.params.nodeID }, function(err, farmerProfile) {
    if (err) {
      return next(new errors.InternalError(err.message));
    }

    if (!farmerProfile) {
      return next(new errors.NotFoundError('Farmer Profile for nodeID not found'));
    }

    res.status(200).send(farmerProfile.toObject()); 
  });
};
...
ContactsRouter.prototype._definitions = function() {
  return [
    ...
    ['GET', '/contacts/profile/:nodeID', this.getProfileByNodeID]
  ];
};
