 ...
 * @param {Mailer} mailer
 */
function ContactsRouter(config, storage, network, mailer) {
  if (!(this instanceof ContactsRouter)) {
    return new ContactsRouter(config, network, storage, mailer);
  }
...
/**
 * Returns the list of connected contacts
 * @private
 */
ContactsRouter.prototype._getConnectedContacts = function() {
  var connected = [];

  for (var index in this.network._router._buckets) {
    connected = connected.concat(
      this.network._router._buckets[index].getContactList()
    );
  }
...
ContactsRouter.prototype.getContactList = function(req, res, next) {
  ...
  let query = {};
  let connected = self._getConnectedContacts().map(function(c) {
    return c.nodeID;
  });
