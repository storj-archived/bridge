...
/**
 * Adds partner referral id
 * @param {http.IncomingMessage} req
 * @param {http.ServerResponse} res
 * @param {Function} next
 */
UsersRouter.prototype.addPartnerReferral = function(req, res, next) {
  const self = this;
  const User = this.storage.models.User;
  const Partner = this.storage.models.Partner;

  log.info('adding partner referral %s', req.body.partner);

  if (!req.body.partner) {
    return next(new errors.BadRequestError('Missing partner data'));
  }

  Partner.findOne({ name: req.body.partner }, function(err, partner) {
    if (err) {
      return next(new errors.InternalError(err.message));
    }
    if (!partner) {
      return next(new errors.NotFoundError('Partner not found'));
    }
    ...
    User.findOne({ _id: req.params.id }, function(err, user) {
      ...
      // Prevents changing initial referrer (?)
      if (user.partnerReferral) {
        return next(new errors.BadRequestError('Partner already exists'));
      }

      user.partnerReferral = partner.id;
      ...
      user.save(function(err) {
        ...
        analytics.identify(req.headers.dnt, {
          userId: user.uuid,
          traits: {
            partnerReferral: partner.id
          }
        });

        analytics.track(req.headers.dnt, {
          userId: user.uuid,
          event: 'Partner Referral Added'
        });
...
UsersRouter.prototype._definitions = function() {
  return [
    ...
    ['PUT', '/users/:id/partner', this._limiter(defaults), this._verify, rawbody, this.addPartnerReferral],
