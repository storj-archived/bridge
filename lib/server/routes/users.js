...
UsersRouter.prototype.createPasswordResetToken = function(req, res, next) {
  ...
  log.info('createPasswordResetToken', self.mailer);
  ...
  User.findOne({ _id: req.params.id }, function(err, user) {
    ...
    log.error('creating password reset token for %s', user._id);
    ...
    user.save(function(err) {
      ...
      console.log('### USER DISPATCH EMAIL', user.resetter, req.body.redirect)
