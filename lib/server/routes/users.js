...
UsersRouter.prototype.createPasswordResetToken = function(req, res, next) {
  ...
  log.info('creating password reset token for %s', req.user._id);
  ...
  User.findOne({ _id: req.params.id }, function(err, user) {
    ...
    user._pendingHashPass = req.body.password;
...
UsersRouter.prototype.confirmPasswordReset = function(req, res, next) {
  ...
  User.findOne({ resetter: req.params.token }, function(err, user) {
    ...
    user.hashpass = user._pendingHashPass;
    user._pendingHashPass = null;
    user.resetter = null;
...
UsersRouter.prototype._definitions = function() {
  return [
    ...
    ['PATCH', '/users/:id', this._verify, this.createPasswordResetToken],
