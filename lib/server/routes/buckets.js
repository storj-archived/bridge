...
BucketsRouter.prototype.updateBucketById = function(req, res, next) {
  ...
  Bucket.findOne({
    ...
    var allowed = ['name', 'pubkeys', 'encryptionKey', 'publicPermissions'];
...
/**
 * Updates the given file's properties
 * @param {http.IncomingMessage} req
 * @param {http.ServerResponse} res
 * @param {Function} next
 */
BucketsRouter.prototype.updateFileById = function(req, res, next) {
  const Bucket = this.storage.models.Bucket;
  const BucketEntry = this.storage.models.BucketEntry;
    ...
    BucketEntry.findOne({
      ...
      if (req.body.filename) {
        if (!entry.index) {
          return next(new errors.UnprocessableEntityError(
            'Cannot rename files with no index'
          ));
        }

        entry.name = req.body.filename;
      }
      ...
      entry.save(function(err) {
        if (err) {
          if (err.code === 11000) {
            return next(
              new errors.ConflictError('Name already used by another file')
            );
          }
        ...
        res.status(200).send(entry.toObject());
      });
    });
  });
};
...
BucketsRouter.prototype._definitions = function() {
  ...
  return [
    ...
    ['PATCH', '/buckets/:id/files/:file', this.getLimiter(limiter(1000)), this._validate, this._verify, this.updateFileById],
