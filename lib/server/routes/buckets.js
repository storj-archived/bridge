...
BucketsRouter.prototype._requestRetrievalPointer = function(item, meta, done) {
  ...
  network.getRetrievalPointer(meta.contact, contract, function(err, dcPointer) {
    ...
    // Record number of bytes downloaded from farmer
    // TODO consider how exchange reports from farmer and client-side
    // should affect this. Bytes downloaded should only be recorded
    // if the download was actually successful.
    var fe = new FarmerEvent({
      nodeID: meta.contact.nodeID,
      timestamp: Date.now(),
      downloadBandwidth: dataSize,
      storage: 0
    });
