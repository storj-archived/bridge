...
BucketsRouter.prototype.getBucketEntryById = function(bucketId, entryId, done) {
  this.storage.models.BucketEntry.findOne({
    _id: entryId,
    bucket: bucketId
  }).populate('frame').exec(function(err, bucket) {
    if (err) {
...
BucketsRouter.prototype.getMirroringTokens = function(mirrorMap, next) {
  ...
  async.mapSeries(mirrorMap, function(mirrorList, done) {
    async.map(mirrorList, self.getMirrorAuthorization, done);
  }, next);
};
/**
...
BucketsRouter.prototype.replicateFile = function(req, res, callback) {
  ...
  async.waterfall([
    ...
    this.getBucketEntryById.bind(this, req.params.id, req.params.file),
    this.getPointersForEntry.bind(this),
    this.getMirrorsForPointers.bind(this),
    this.getMirroringTokens.bind(this),
    this.createMirrorsFromTokenMap.bind(this)
  ],
...
BucketsRouter.prototype._getPointersFromEntry = function(entry, opts, done) {
  ...
  cursor.exec(function(err, pointers) {
    if (err) {
      return done(new errors.InternalError(err));
    }
