...
BucketsRouter.prototype.replicateFile = function(req, res, callback) {
  ...
  this._getBucketById(req.params.id, req.user._id, function(err) {
    if (err) {
      return callback(err);
    }

    let didRespondToClient = false;

    async.waterfall([
      self.getBucketEntryById.bind(self, req.params.id, req.body.file),
      self.getPointersForEntry.bind(self),
      self.getMirrorsForPointers.bind(self),
      // NB: Once we have the mirror map, respond to client instead of
      // NB: waiting to establish - result can be checked later by client
      (mirrorMap, next) => {
        didRespondToClient = true;
        res.status(202).send(mirrorMap.map((mirrors) => {
          return mirrors.map((mirror) => mirror.toObject());
        }));
        next(null, mirrorMap);
      },
      self.getMirroringTokens.bind(self),
      self.createMirrorsFromTokenMap.bind(self)
    ], (err) => {
      /* istanbul ignore else */
      if (err && !didRespondToClient) {
        callback(err);
      }
    });
  });
...
BucketsRouter.prototype.listMirrorsForFile = function(req, res, next) {
  ...
  function _getMirrorsFromHashes(hashes, callback) {
    async.map(hashes, function(hash, next) {
      self.storage.models.Mirror.find({ shardHash: hash }, (err, mirrors) => {
        if (err) {
          return callback(err);
        }
  ...
  async.waterfall([
    this._getBucketById.bind(this, req.params.bucket, req.user._id),
    _getFrameForFile,
    _getHashesFromFrame,
...
BucketsRouter.prototype._getRetrievalToken = function(sPointer, opts, done) {
  ...
  this.contracts.load(sPointer.hash, function(err, item) {
    ...
    var meta = {
      farmers: Object.keys(item.contracts).filter(function(nodeID) {
    ...
    function tokenIsResolvedOrFarmersExhausted() {
      return typeof meta.token === 'string' || meta.farmers.length === 0;
    }

    function handleResults(err, pointer) {
      if (!meta.token) {
        return done(new Error('Failed to get retrieval token'));
      }

      pointer.size = sPointer.size;

      done(null, pointer);
    }

    async.until(
      tokenIsResolvedOrFarmersExhausted,
      self._requestRetrievalPointer.bind(self, meta),
      handleResults
    );
  });
};
 ...
 * @private
 * @param {Object} meta
 * @param {Array} meta.farmers - List of farmer NodeIDs
 * @param {StorageItem} meta.item - Loaded storage item data
 * @param {String} meta.token - Retreival token from farmer
 * @param {BucketsRouter~_requestRetrievalPointerCallback}
 */
BucketsRouter.prototype._requestRetrievalPointer = function(meta, done) {
  const Contact = this.storage.models.Contact;
  const contracts = this.contracts;
  const network = this.network;

  Contact.findOne({ _id: meta.farmers.shift() }, function(err, contact) {
    if (err) {
    ...
    let contract = meta.item.contracts[farmer.nodeID];
    ...
    network.getRetrievalPointer(farmer, contract, function(err, dcPointer) {
      ...
      if (!meta.item.meta[farmer.nodeID]) {
        meta.item.addMetaData(farmer, { downloadCount: 0 });
      }

      if (!meta.item.meta[farmer.nodeID].downloadCount) {
        meta.item.meta[farmer.nodeID].downloadCount = 0;
      }

      meta.item.meta[farmer.nodeID].downloadCount++;

      contracts.save(meta.item, function(err) {
        if (err) {
 ...
 */
BucketsRouter.prototype._getPointersFromEntry = function(entry, opts, done) {
  const self = this;
  const Pointer = this.storage.models.Pointer;

  let pQuery = { _id: { $in: entry.frame.shards } };
  let pSkip = Number(opts.skip) || 0;
  let pLimit = Number(opts.limit) || 6;
  let pSort = { index: 1 };
  ...
  cursor.exec(function(err, pointers) {
    ...
    async.mapSeries(pointers, function(sPointer, next) {
...
BucketsRouter.prototype._definitions = function() {
  return [
    ...
    ['GET', '/buckets/:id/mirrors', this._verify, this.listMirrorsForFile]
  ];
};
