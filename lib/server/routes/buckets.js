...
BucketsRouter.prototype.getFile = function(req, res, next) {
  ...
  Bucket.findOne({
  ...
  }).populate('user').exec(function(err, bucket) {
    if (err) {
      return next(new errors.InternalError(err.message));
    }
    ...
    if (bucket.user.isDownloadRateLimited(rates.hourlyBytes,
                                          rates.dailyBytes,
                                          rates.monthlyBytes)) {

      log.warn('getFile: Transfer rate limited, user: %s', bucket.user.email);
    ...
    BucketEntry.findOne({
      ...
      const user = bucket.user.recordDownloadBytes(entry.frame.size);
      user.save((err) => {
        if (err) {
