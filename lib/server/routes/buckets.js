...
BucketsRouter.prototype.destroyBucketById = function(req, res, next) {
  ...
  BucketEntry.findOne({ bucket: req.params.id }).populate({
    path: 'frame',
    populate: {
      path: 'shards'
    }
  }).exec((err, entry) => {
    let shardHashes = entry.frame.shards.map((shard) => {
      return shard.hash;
    });

    FullAudit.removeFullAuditByHash(shardHashes);
    BucketEntry.remove({ bucket: req.params.id });
  });
...
BucketsRouter.prototype.createEntryFromFrame = function(req, res, next) {
  ...
  const FullAudit = this.storage.models.FullAudit;
  ...
  Bucket.findOne({
    ...
    Frame.findOne({
      ...
      let shardHashes = [];
      frame.shards.forEach((pointer) => {
        shardHashes.push(pointer.hash);
      });

      FullAudit.scheduleFullAuditsFromShard(Shard, shardHashes, (err) => {
        if(err) {
          return next(new errors.InternalError(err.message));
        }
