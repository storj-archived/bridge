...
BucketsRouter.prototype.getFile = function(req, res, next) {
  ...
  Bucket.findOne({
    ...
    User.findOne({
      ...
      let rates = self.config.application.freeTier.down;
      if (!user.isFreeTier) {
        rates = self.config.application.tierOne.down;
      }
...
BucketsRouter.prototype._definitions = function() {
  ...
  return [
    ...
    ['GET', '/buckets/:id/files/:file/mirrors', this._limiter(defaults), this._validate, this._verify, this.listMirrorsForFile]
  ];
};
