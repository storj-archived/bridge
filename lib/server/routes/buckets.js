...
function BucketsRouterFactory(config, storage, network) {
  ...
  function createBucketToken(req, res, next) {
    ...
    function authorize(callback) {
      ...
      let rawbody = verify[0];
      let checkauth = verify[1];
    ...
    Bucket.findOne({
      ...
      let transfer = through(
        function onData(chunk) {
          var self = this;
          var duration = ms(constants.DEFAULT_FILE_TTL);

          self.pause();
          network.store(chunk, duration, function(err, hash) {
            if (err) {
  ...
  return [
    ...
    ['POST'   , '/buckets/:id/tokens'       , /* NONE */ createBucketToken],
