    ...
    Frame.findOne({
      ...
      entry.save(function(err) {
        ...
        frame.lock(function(err) {
          ...
          self._replicateFrameToMirrors(frame);
        });
      });
    });
  });
 ...
 * @private
 * @param {Frame} frame - The frame to replicate
 */
BucketsRouter.prototype._replicateFrameToMirrors = function(frame) {
  const self = this;
  ...
  function _getMirroringContract(contract, n, next) {
    network._manager.load(contract.get('data_hash'), function(err, item) {
      ...
      Contact.findOne({
        ...
        network.getStorageOffer(contract, function(mirror, contract) {
          item.contracts[mirror.nodeID] = contract;
          ...
          network._manager.save(item, function(err) {
            ...
            network.getRetrieveToken(farmer, contract, function(err, token) {
              ...
              mirrors.push(mirror);
              next(null, new storj.DataChannelPointer(
                farmer,
  ...
  function _replicateShard(contract, next) {
    async.timesSeries(
      self.config.application.mirrors,
      _getMirroringContract.bind(contract),
  ...
  async.mapSeries(
    ...
    function(err, contracts) {
      ...
      async.eachSeries(contracts, _replicateShard, function(err) {
        if (err) {
          return log.error(err.message);
        }

        log.info('Successfully replicated frame %s', frame.id);
      });
    }
  );
};
  ...
  Bucket.findOne({
    ...
    BucketEntry.findOne({
      ...
      cursor.exec(function(err, pointers) {
        ...
        async.map(pointers, getRetrievalToken, function(err, results) {
          if (err) {
            return next(new errors.InternalError(err.message));
          }
