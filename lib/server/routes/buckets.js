...
BucketsRouter.prototype.getFile = function(req, res, next) {
  ...
  const rates = this.config.application.freeTier.down;
  if (req.user.isDownloadRateLimited(rates.hourlyBytes,
                                     rates.dailyBytes,
                                     rates.monthlyBytes)) {

    log.warn('getFile: Transfer rate limited, user: %s', req.user.email);
  ...
  Bucket.findOne({
    ...
    BucketEntry.findOne({
      ...
      if (!Number.isFinite(entry.frame.size)) {
        log.warn('getFile: frame %s size %s is not a finite number',
                 entry.frame._id, entry.frame.size);
        return next(new errors.InternalError(
          'Frame size in not a finite number'));
      }

      const user = req.user.recordDownloadBytes(entry.frame.size);
      user.save((err) => {
        if (err) {
          log.warn('getFile: unable to update downloaded bytes %s',
                   entry.frame.size);
        }
      });
