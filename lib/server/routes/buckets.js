...
const auditor = require('../../audit/auditor.js');
const ms = require('ms');
...
function BucketsRouter(config, storage, network, mailer) {
  ...
  this._auditor = new Auditor(config.audits.redis);
}
...
BucketsRouter.prototype.createEntryFromFrame = function(req, res, next) {
  ...
  function continue(err, results) {
    var bucket = results[0];
    var frame = results[1];

    async.parallel([
      async.apply(saveEntry, bucket, frame),
      async.apply(createAuditSchedule, frame)
    ],
    function done(err , result) {
      let entry = result[0];
      res.send(Object.assign(entry.toObject(), { size: frame.size }));
    });
  }

  function findBucket(cb) {
    Bucket.findOne({
      user: req.user._id,
      _id: req.params.id
    }, function(err, bucket) {
      ...
      if (!bucket) {
        return cb(new errors.NotFoundError('Bucket not found'));
      }
  ...
  function findFrame(cb) {
    Frame.findOne({
      ...
      if (!frame) {
        return cb(new errors.NotFoundError('Frame not found'));
      }
  ...
  function saveEntry(bucket, frame, cb) {
    ...
    entry.save(function(err) {
      ...
      frame.lock(function(err) {
        if (err) {
          return cb(new errors.InternalError(err));
        }
  ...
  function createAuditSchedule(frame, cb) {
    var audits = [];
    ...
    frame.shards.forEach(function(shard) {
      self.network._manager.load(shard.hash, function(err, item) {
        ...
        Object.keys(item.challenges).forEach(function(key) {
          let eTime = item.contracts[key].start_time;
          let sTime = item.contracts[key].end_time;
          ...
          item.challenges[key].challenges.forEach(function(challenge, ind) {
            ...
            audits.push(auditJob);
          });
        });
      });
    });

    this._auditqueue.add(audits, function(err, count) {
      if(err) {
        return callback(err);
      }
