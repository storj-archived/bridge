...
FramesRouter.prototype._getContractForShard = function(contr, audit, bl, res, done) {
  ...
  this._selectFarmers(bl, (err, farmers) => {
    ...
    this._publishContract(farmers, contr, audit, (err, data) => {
      if (err) {
        return done(new errors.InternalError(err.message));
      }
  ...
  async.parallel([
    (next) => {
      this.storage.models.Contact.find({
        responseTime: { $lte: constants.PUBLISH_BENCH_THRESHOLD },
        _id: { $nin: excluded }
      }).sort({ lastPublishSent: 1 })
        .limit(constants.PUBLISH_TOTAL)
        .exec(next);
    },
    (next) => {
      this.storage.models.Contact.find({
        responseTime: { $gt: constants.PUBLISH_BENCH_THRESHOLD },
        _id: { $nin: excluded }
      }).sort({ lastPublishSent: 1 })
        .limit(constants.PUBLISH_BENCH_TOTAL)
        .exec(next);
    }
  ], (err, results) => {
    if (err) {
...
FramesRouter.prototype._publishContract = function(nodes, contract, audit, callback) {
  ...
  this.contracts.load(hash, (err, item) => {
    ...
    this.network.publishContract(nodes, contract, (err, offered) => {
      if (err) {
        return callback(err);
      }

      item.addContract(offered.contact, offered.contract);
      item.addAuditRecords(offered.contact, audit);
    ...
    function getContract(done) {
      ...
      self.storage.models.Mirror.find({
        ...
        if (!mirror) {
          ...
          log.info('Requesting contract for frame: %s, shard hash: %s and size: %s',
                   req.params.frame, req.body.hash, req.body.size);

          self._getContractForShard(contr, audit, bl, res, function(err, data) {
            if (err) {
              ...
              farmer = storj.Contact(data.contact);
              contract = storj.Contract(data.contract);
              token = data.token;
              done();
            }
          });
