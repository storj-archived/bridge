    ...
    function getContract(done) {
      ...
      self.storage.models.Mirror.find({
        ...
        if (!mirrors || !mirrors.length) {
          ...
          self._getContractForShard(contr, audit, bl, function(err, _farmer, _contract) {
            if (err) {
          ...
          farmer = storj.Contact(filtered[0].contact);
          contract = storj.Contract(filtered[0].contract);
          done();
        }
      });
    },
