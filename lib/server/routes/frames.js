...
FramesRouter.prototype._getContractForShardSIP6 = function(contr, audit, bl, res, done) {
  this._selectFarmers(bl, (err, farmers) => {
    ...
    // Mark farmers/contacts that haven't been seen within the last day as having no space available
    for (var i = 0; i < farmers.length; i++) {
      if (farmers[i].lastSeen < new Date(Date.now() - 864e5) {
        farmers[i].spaceAvailable = false;
        // load contact object from db
        // set space available to false
        // save to db
      }
    }
    farmers = farmers.filter(function(farmer) { farmer.spaceAvailable === true });
    
    // pass the updated list of only farmers with spaceavailable:true here 
