  ...
  function _getRetrievalTokenFromFarmer(mirror, callback) {
    self.contracts.load(hash, (err, item) => {
      ...
      async.until(test, (done) => {
        ...
        callback(null, pointer, mirror, item);
      });
    });
  }

  function _establishMirror(source, mirror, item, callback) {
    self.network.getMirrorNodes([source], [mirror.contact], (err) => {
      if (err) {
        return callback(err);
      }

      item.addContract(destination, storj.Contract(mirror.contract));
