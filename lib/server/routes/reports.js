  ...
  function _establishMirror(source, mirror, contact, item, callback) {
    self.network.getMirrorNodes(
      ...
      (err) => {
        ...
        self.contracts.save(item, (err, result) => {
          if (err) {
            return callback(err);
          }

          callback(null, source, mirror, contact, item, result);
        });
      }
    );
  }

  function _addMirrorFullAudit(
    pointer,
    mirror,
    contact,
    item,
    ...
    //this process will need refactoring, it's currently re-using audits for
    //mirrors
    const FullAudit = self.storage.models.FullAudit;
    const storjItem = item.toObject();
    let audit;
    try {
      audit = storj.AuditStream.fromRecords(
        storjItem.challenges[Object.keys(storjItem.challenges)[0]],
        storjItem.trees[Object.keys(storjItem.challenges)[0]]
      );
    } catch(err) {
      return callback(err);
    }

    let auditData = audit.getPrivateRecord();

    FullAudit.scheduleFullAudits([{
      start: mirror.contract.store_begin,
      end: mirror.contract.store_end,
      farmer_id: mirror.contract.farmer_id,
      data_hash: mirror.contract.data_hash,
      root: auditData.root,
      depth: auditData.depth,
      challenges: auditData.challenges
    }], null, (err) => {
      if(err) {
        return callback(new Error('Failed to generate audits'));
      }
