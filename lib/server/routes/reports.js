...
ReportsRouter.prototype.createExchangeReport = function(req, res, next) {
  ...
  this.storage.models.StorageEvent.findOne({token: token}, (err, event) => {
    ...
    if (event.processed) {
      return next(new errors.NotAuthorizedError('Storage event closed'));
    }
    ...
    if (isClientReport && !hasClientReport) {
       ...
       * IMPORTANT
       *
       * There is currently a chain of events from the initial upload, to
       * the next farmer and continued, and if any of that does not succeed,
       * the mirroring processs halts.
       *
       * Ideally this process of kicking off the mirroring would be done
       * as a seperate service. We would only attach the reports to the storage
       * event here and that way another service can look at the contracts
       * for each shard and then determine if there are additional mirrors
       * that are needed for the shard. This would be a much more robust
       * method for creating replicas.
       */
...
ReportsRouter.prototype._triggerMirrorEstablish = function(n,
  ...
  function _getRetrievalTokenFromFarmer(mirror, callback) {
    ...
    /**
     * IMPORTANT
     * We are only mirroring to one selected mirror canidate
     * and not attempting to try multiple. This is fragile.
     * Ideally it would try multiple combinations of sources
     * and destinations as similar to the method for replication
     * used in storj-monitor.
     */

    async.until(test, (done) => {
      let sourceNodeID = farmers.pop();
      // Do not try to mirror to the same farmer
      if (sourceNodeID === mirror.contact.nodeID) {
        return done();
      }

      self.getContactById(sourceNodeID, (err, result) => {
        if (err) {
          return done();
        }
