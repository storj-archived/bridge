...
const paymentProcessors = {
  stripe: {
    add: (options) => {
      console.log("payment-processor-adapter line 10 ======= ");
      return new Promise((resolve, reject) => {
        console.log("payment-processor-adapter Promise =======");
        stripe.customers.create({
          source: options.data, // token obtained with Stripe.js
          plan: 'gold_member',
          email: options.user
        }, function(err, customer) {
          console.log("stripe callback line 18 payment-processor-adapter");
          console.log("stripe callback err: ", err);
          console.log("stripe callback customer: ", customer); 
          if (err) {
            return reject(err);
          }
