...
shortdesc:
	echo 'build	- `down` docker <compositions> and <service> (optional) using given <env>' | fold -s
...
%:
	if [ $(firstword $(MAKECMDGOALS)) != $@ ]; then \
		if [ $(words $(MAKECMDGOALS)) -lt 4 ]; then \
			docker-compose -f dockerfiles/$(word 2, $(MAKECMDGOALS))-$(word 3, $(MAKECMDGOALS)).yml down \
		else; \
			docker-compose -f dockerfiles/$(word 2, $(MAKECMDGOALS))-$(word 3, $(MAKECMDGOALS)).yml down $(word 4, $(MAKECMDGOALS)) \
		fi; \
	fi
